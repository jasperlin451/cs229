...
for i in range(5,1000,5):
  # i = 65, 110 seem to be peaks
  ada = AdaBoostClassifier(n_estimators=i)
#cutoff = int(round(len(X)*.8))
#ada.fit(X[0:cutoff],y[0:cutoff])
#print ada.score(X[0:cutoff],y[0:cutoff])
#print ada.score(X[cutoff:len(X)],y[cutoff:len(y)])
  skf = cross_validation.StratifiedKFold(y, n_folds=k, shuffle=True)
  scores = cross_validation.cross_val_score(ada, X, y, cv=skf)
  print i
  print sum(scores)/k"""
