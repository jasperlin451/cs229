...
cutoff = int(round(len(X)*.7))
print cutoff
randomForest = RandomForestClassifier(n_estimators=100).fit(X[0:cutoff], y[0:cutoff])
print randomForest.score(X[cutoff:len(X)],y[cutoff:len(y)])
